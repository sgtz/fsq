...
{
    // code from this file originally comes from http://code.kx.com/wsvn/code/kx/kdb%2B/c/c.cs, and provided originally courtesy of Barclays Capital.

    // TODO: update to revision Rev 1703 + phase out extensions that are no longer necessary
    // TODO: harmonise namespaces of types like KTimeSpan, Date, etc.
    ...
    namespace q
    {
        public partial class c : TcpClient
        {
            ...
            */
            ...
            static void O(object x) { Console.WriteLine(x); }internal static string i2(int i) { return String.Format("{0:00}", i); }
            public static int ni = Int32.MinValue; public static long nj = Int64.MinValue, o = (long)8.64e11 * 730119; public static double nf = Double.NaN;
 ...
 new Month(ni),new Date(ni),new DateTime(0),new KTimespan(nj),new Minute(ni),new Second(ni),new TimeSpan(nj)};
            public object NULL(char c) { return NU[" bg xhijefcspmdznuvt".IndexOf(c)]; }
            ...
            // TODO: does qn make sense for special values?
            private void u()
            {
                int n = 0, r = 0, f = 0, s = 8, p = s; short i = 0; j = 0; byte[] dst = new byte[ri()]; int d = j; int[] aa = new int[256];
                while (s < dst.Length)
                {
                    if (i == 0) { f = 0xff & (int)b[d++]; i = 1; } if ((f & i) != 0) { r = aa[0xff & (int)b[d++]]; dst[s++] = dst[r++]; dst[s++] = dst[r++]; n = 0xff & (int)b[d++]; for (int m = 0; m < n; m++)dst[s + m] = dst[r + m]; } else dst[s++] = b[d++];
                    while (p < s - 1) aa[(0xff & (int)dst[p]) ^ (0xff & (int)dst[p + 1])] = p++; if ((f & i) != 0) p = s += n; i *= 2; if (i == 256) i = 0;
                }
                b = dst; j = 8;
            }
            public static DateTime za = DateTime.MinValue.AddTicks(1), zw = DateTime.MaxValue;  // change
            ...
            void w(DateTime p) { if (vt < 1)throw new KException("Timestamp not valid pre kdb+2.6"); w((p == ap) ? aj : p == wp ? wj : qn(p) ? nj : (100 * (p.Ticks - o))); }
            ...
            DateTime rp()
            {
                ...
                {
                    long d;
                    d = j < 0 ? (j + 1) / 100 - 1 : j / 100;
                    p = new DateTime(j == nj ? 0 : o + d);
                }
                return p;
            }
            void w(object x)
            ...
            object r()
            {
                ...
                //if (t == 99) {
                //    var d=new Dict(r(), r());
                //    if (d.x is Flip && d.y is Flip)
                //    {
                //        return new Tbl((Flip)d.x, (Flip)d.y);
                //    }
                //    else
                //    {
                //        return d;
                ...
                //}
                j++; 
                if (t == 98) return new Flip((Dict)r()); n = ri();
                switch (t)
                {
            ...
            public void ks(String s, Object x, Object y, Object z) { Object[] a = { cs(s), x, y, z }; w(0, a); }
        }

        [Serializable]
        public partial class Second : IComparable
        {
            public int i; private Second() { }public Second(int x) { i = x; }
            public override string ToString() { return i == c.ni ? "" : new Minute(i / 60).ToString() + ':' + c.i2(i % 60); }
        ...
        [Serializable]
        public partial class Minute : IComparable
        {
            public int i; private Minute() { }public Minute(int x) { i = x; }
            public override string ToString() { return i == c.ni ? "" : c.i2(i / 60) + ":" + c.i2(i % 60); }
        ...
        public class Flip { public string[] x; public object[] y; public Flip(Dict X) { x = (string[])X.x; y = (object[])X.y; } public object at(string s) { return y[c.find(x, s)]; } public object at<T>(string s) { return (T) at(s); } }

        [Serializable]
        public partial class KTimespan : IComparable
        {
            public TimeSpan t; private KTimespan() { }public KTimespan(long x) { t = new TimeSpan(x == c.nj ? c.nj : x / 100); }
            public override string ToString() { return c.qn((object)t) ? "" : t.ToString(); }
        ...
        {
            public object x; public object y; public Dict(object X, object Y) { x = X; y = Y; }
            public object[] Y
            {
                get
                {
        ...
        [Serializable]
        public partial class Month : IComparable
        {
            public int i; private Month() { }public Month(int x) { i = x; }
            public override string ToString() { int m = 24000 + i, y = m / 12; return i == c.ni ? "" : c.i2(y / 100) + c.i2(y % 100) + "-" + c.i2(1 + m % 12); }
        ...
        [Serializable]
        public partial class Date : IComparable
        {
            public int i; private Date() { }public Date(int x) { i = x; }
            public DateTime DateTime() { return i == -Int32.MaxValue ? c.za : i == Int32.MaxValue ? c.zw : new DateTime(i == c.ni ? 0L : c.clampDT((long)8.64e11 * i + c.o)); }
            public Date(long x) { i = x == 0L ? c.ni : (int)(x / (long)8.64e11) - 730119; }public Date(DateTime z) : this(z.Ticks) { }
            public override string ToString() { return i == c.ni ? "" : this.DateTime().ToString("d"); }
